...
# analyses
set_list = list(
	peptidergic = list(ann_fn = "../results_scatlas/results_metacell_it4_peptidergic/", icc_fn = "results_alignment_icc_peptidergic/", ctt_sprtinf_string = "%s/annotation_mc.%s.it4.peptidergic.tsv", focus_list = list("cts" = "cell_type", "mcs" = "metacell")),
	global =      list(ann_fn = "../results_scatlas/results_metacell_it4/"            , icc_fn = "results_alignment_icc/",             ctt_sprtinf_string = "%s/annotation_mc.%s.it4.reordered.tsv",   focus_list = list("cts" = "cell_type", "bct" = "broad_cell_type"))
)


for (s in 1:length(set_list)) {
	
	set_id = names(set_list)[s]
	ann_fn = set_list[[s]]$ann_fn
	icc_fn = set_list[[s]]$icc_fn
	ctt_sprtinf_string = set_list[[s]]$ctt_sprtinf_string
	focus_list = set_list[[s]]$focus_list
	...
	for (i in 1:length(focus_list)) {
		...
		boo_total = num_bs
		boo_list = NULL
		while (length(boo_list) < boo_total) {
			ali_f_boo = tryCatch( phangorn::bootstrap.phyDat(ali_f, parsimony_fun, multicore = FALSE, bs = 1), error = function(e) e)
			if (!inherits(ali_f_boo, "simpleError")) {
				message(sprintf("bootstrap try: %i/%i bootstraps", length(boo_list), boo_total))
				if (boo_iter == 1) {
					boo_list = ali_f_boo
				} else {
					boo_list = c(boo_list, ali_f_boo[[1]])
				}
				boo_iter = boo_iter + 1
			}
		}
