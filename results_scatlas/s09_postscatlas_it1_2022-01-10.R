...
for (spi in sps_list) {
	...
	metacell::scdb_add_mc(sprintf("%s_it1_ord",run_name), mc = mc)
	mc = metacell::scdb_mc(sprintf("%s_it1_ord",run_name))
	...
	metacell::mcell_mc2d_force_knn(
		mc2d_id = sprintf("%s_it1",run_name),
		mc_id = sprintf("%s_it1",run_name), 
		feats_gset = sprintf("%s_it1",run_name), 
		graph_id = sprintf("%s_it1",run_name))
	mc2d = metacell::scdb_mc2d(sprintf("%s_it1",run_name))
	...
	write.table(mc_t, file = sprintf("%s/annotation_mc.%s.it1.tsv", out_fn, spi), sep = "\t", quote = FALSE, row.names = FALSE)
