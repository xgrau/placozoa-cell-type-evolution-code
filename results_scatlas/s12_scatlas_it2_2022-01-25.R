...
for (spi in sps_list) {
	...
	# the `it2` matrix is already filtered after it0; all removed cells are lised in mat@ignore_cells
	...
	metacell::mcell_add_gene_stat(
		gstat_id = sprintf("%s_it2", run_name),
		mat_id = sprintf("%s_it2", run_name),
		force = TRUE)
	...
	metacell::mcell_gset_filter_multi(
		gstat_id = sprintf("%s_it2", run_name),
		gset_id = sprintf("%s_it2", run_name),
		T_tot = 100,
		T_top3 = 2,
		T_szcor = -0.05,
		T_niche = 0.01,
	...
	# 	new_mc_id = sprintf("%s_it2_split", run_name),
	# 	mc_id = sprintf("%s_it2", run_name), 
	# 	mat_id = sprintf("%s_it2", run_name),
	# 	T_lfc = 8
	# )
	...
	mc = metacell::scdb_mc(id = sprintf("%s_it2", run_name))
	mat = metacell::scdb_mat(id = sprintf("%s_it2", run_name))
	...
	metacell::scdb_add_mc(id = sprintf("%s_it2",run_name), mc_reord)
